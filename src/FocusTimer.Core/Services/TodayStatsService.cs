...
public sealed class TodayStatsService
{
    ...
    public TodayStatsService(IWorklogStore worklogStore, IAppLogger logger) { this._worklogStore = worklogStore; this._logger = logger; }
    /// <summary>Refreshes the total using the local-day half-open interval.</summary>
    public async Task RefreshTodayAsync()
    {
        try
        { var date = DateTime.Today; var start = new DateTimeOffset(date, TimeZoneInfo.Local.GetUtcOffset(date)); var read = await this._worklogStore.QueryAsync(new WorklogQuery(start, start.AddDays(1))); if (!read.Outcome.IsSuccess) { this._logger.LogWarning(read.Outcome.Message ?? "Unable to read worklog."); return; } this._today = date; this._todayTotal = read.Entries.Aggregate(TimeSpan.Zero, (sum, entry) => sum + entry.Duration); }
    ...
    public async Task AddEntriesAsync(IEnumerable<TimeEntry> entries) { if (this._today != DateTime.Today) { await this.RefreshTodayAsync(); return; } this._todayTotal += entries.Where(e => e.StartedAt.Date == this._today).Aggregate(TimeSpan.Zero, (sum, entry) => sum + entry.Duration); }
